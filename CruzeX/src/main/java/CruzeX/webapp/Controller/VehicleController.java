...
private void updateVehicle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
 ...
 private void addVehicle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try {
        // For multipart/form-data, get parameters from parts
        String vehicleID = getParameterFromPart(request, "vehicleID");
        String vehicleName = getParameterFromPart(request, "vehicleName");
        String category = getParameterFromPart(request, "category");
        String monthFeeStr = getParameterFromPart(request, "monthFee");
        String driverIDStr = getParameterFromPart(request, "driverID");

        // Validate required fields
        if (vehicleID == null || vehicleID.trim().isEmpty() || 
            vehicleName == null || vehicleName.trim().isEmpty() || 
            category == null || category.trim().isEmpty()) {
            message = "Vehicle ID, name, and category are required.";
        } else {
            // Parse numbers
            int monthFee = 0;
            int driverID = 0;
            ...
            try {
                monthFee = Integer.parseInt(monthFeeStr);
                driverID = Integer.parseInt(driverIDStr);
            } catch (NumberFormatException e) {
                message = "Invalid input for price or staff ID.";
                request.setAttribute("message", message);
            ...
            if (filePart != null && filePart.getSize() > 0) {
                ...
                // Define upload path
                String uploadPath = getServletContext().getRealPath("/img/car");
                File uploadDir = new File(uploadPath);
                if (!uploadDir.exists()) {
            ...
            message = result ? "Book '" + vehicleName + "' added successfully!" : "Failed to add book.";
        }
    } catch (ClassNotFoundException | SQLException e) {
...
// Helper method to get parameter value from multipart request
private String getParameterFromPart(HttpServletRequest request, String paramName) throws IOException, ServletException {
